...
public class FieryStockpotBlockEntityRender implements BlockEntityRenderer<FieryStockpotBlockEntity> {
    private final BlockEntityRendererProvider.Context context;
    private final Function<ResourceLocation, ISoupBaseRender> soupBaseRender;

    public FieryStockpotBlockEntityRender(BlockEntityRendererProvider.Context context) {
    ...
    @Override
    public void render(@NotNull FieryStockpotBlockEntity stockpot, float partialTick, @NotNull PoseStack poseStack, @NotNull MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        if (status == FieryStockpotBlockEntity.PUT_SOUP_BASE) {
            return;
        }
        ...
        if (status == FieryStockpotBlockEntity.PUT_INGREDIENT) {
        ...
        } else if (status == FieryStockpotBlockEntity.COOKING) {
        ...
        } else if (status == FieryStockpotBlockEntity.FINISHED) {
            StockpotVisuals visuals = Objects.requireNonNullElse(stockpot.visuals, StockpotVisuals.DEFAULT);
            int takeoutCount = stockpot.getTakeoutCount();
            int maxCount = Math.min(stockpot.getResult().getCount(), FieryStockpotBlockEntity.MAX_TAKEOUT_COUNT);
    ...
    private void renderItems(FieryStockpotBlockEntity stockpot, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay, boolean isFinished) {
