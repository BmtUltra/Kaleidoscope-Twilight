    ...
    public static void init() {
        ...
        NAGA_TEA = registry.registerTeacupData("naga_tea",
                TeacupRegistry.TeacupData.create(4)
                        .addEffect(() -> new MobEffectInstance(KTEffects.STURDY_SCALES.get(), 2 * 60 * 20))
        );

        // 巫术茶
        WITCHCRAFT_TEA = registry.registerTeacupData("witchcraft_tea",
                TeacupRegistry.TeacupData.create(4)
                        .addEffect(() -> new MobEffectInstance(KTEffects.WITCHCRAFT_PROTECTION.get(), 2 * 60 * 20))
        );

        // 米诺菇茶
        MINOTAUR_MUSHROOM_TEA = registry.registerTeacupData("minotaur_mushroom_tea",
                TeacupRegistry.TeacupData.create(4)
                        .addEffect(() -> new MobEffectInstance(KTEffects.MUSHROOM_PERCEPTION.get(), 5 * 60 * 20))
        );

        // 火焰茶
        FIRE_TEA = registry.registerTeacupData("fire_tea",
                TeacupRegistry.TeacupData.create(4)
                        .addEffect(() -> new MobEffectInstance(KTEffects.FIRE_BREATH.get(), 3 * 60 * 20))
                        .addEffect(() -> new MobEffectInstance(ModEffects.WARMTH.get(), 5 * 60 * 20))
        );

        // 幻影茶
        PHANTOM_TEA = registry.registerTeacupData("phantom_tea",
                TeacupRegistry.TeacupData.create(4)
                        .addEffect(() -> new MobEffectInstance(KTEffects.PHANTOM.get(), 2 * 60 * 20))
        );
        ...
        ICE_CRYSTAL_TEA = registry.registerTeacupData("ice_crystal_tea",
                TeacupRegistry.TeacupData.create(4)
                        .addEffect(() -> new MobEffectInstance(KTEffects.FROST_CLOUD.get(), 60 * 20))
                        .addEffect(() -> new MobEffectInstance(KTEffects.YETI_THROW.get(),  60 * 20))
        );
    }
